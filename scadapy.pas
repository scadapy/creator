...
type
  ...
  TMainFrame = class(TForm)
    ...
    procedure VkTreeDragOver(Sender, Source: TObject; X, Y: Integer;
      State: TDragState; var Accept: Boolean);
...
begin
       Version:='ScadaPy Creator v.3.3.9 ';
...
//////////////////////////////////////////////////////
       ...
       //////////////////////////////////////////////////////
              ...
              StringGrid3.Cells[1,0] := 'Название переменной';
              StringGrid3.Cells[2,0] := 'Сообщение';
              StringGrid3.Cells[3,0] := 'vk id';
              StringGrid3.Cells[4,0] := 'Период отправки';

              StringGrid3.ColWidths[0]:=30;
              StringGrid3.ColWidths[1]:=200;
              StringGrid3.ColWidths[2]:=280;
...
procedure TMainFrame.BitBtn16Click(Sender: TObject);
begin
  {$IFDEF Windows}
...
procedure TMainFrame.MenuItem22Click(Sender: TObject);
begin
  VkClient();
end;

procedure TMainFrame.MenuItem23Click(Sender: TObject);
begin
   {$IFDEF Windows}
      RunCmdCommand(PathToPythonWindows+' '+ProjectPath+'vkclient.py"');
   {$ENDIF Windows}
...
procedure TMainFrame.StringGrid3DragDrop(Sender, Source: TObject; X, Y: Integer
  );
var
  DestCol, DestRow: Integer;
begin
   ...
   if Source is TTreeView then
     begin
          ...
          StringGrid3.RowCount:=StringGrid3.RowCount+1;
          StringGrid3.MouseToCell(X, Y, DestCol, DestRow);
          StringGrid3.Cells[DestCol,DestRow]:=VkTree.Selected.Text;

     end;
end;

procedure TMainFrame.StringGrid3DragOver(Sender, Source: TObject; X,
  Y: Integer; State: TDragState; var Accept: Boolean);
begin
  Accept := true;
end;

procedure TMainFrame.StringGrid3SelectCell(Sender: TObject; aCol,
  aRow: Integer; var CanSelect: Boolean);
begin
      ArRow:=ARow;
end;

procedure TMainFrame.StringGrid3SelectEditor(Sender: TObject; aCol,
  aRow: Integer; var Editor: TWinControl);
begin
...
procedure TMainFrame.TabSheet3Show(Sender: TObject);
begin
      LoadVkTree();
end;
...
procedure TMainFrame.VkTreeDragOver(Sender, Source: TObject; X, Y: Integer;
  State: TDragState; var Accept: Boolean);
begin
  Accept := true;
end;
...
begin
            ...
            begin
              ...
              //   z:=0;
               ...
               //  Memo1.Lines.Add('     unitCount='+z.ToString);
                 ...
                 Memo1.Lines.Add('                 sock_udp.sendto(  json.dumps( {''name'':varname[i],''data'':varNameData} ).encode(''utf-8''), server_address_udp)');
                 Memo1.Lines.Add('                 sock_udp_arch.sendto(  json.dumps( {''name'':varname[i],''data'':varNameData} ).encode(''utf-8''), server_address_udp_arch)');
                 Memo1.Lines.Add('                 sock_udp_vk.sendto(  json.dumps( {''name'':varname[i],''data'':varNameData} ).encode(''utf-8''), server_address_udp_vk)');

                 Memo1.Lines.Add('             except Exception as e:');
                 Memo1.Lines.Add('                 varNameData=None');

                 Memo1.Lines.Add('                 sock_udp.sendto(  json.dumps( {''name'':varname[i],''data'':''Error''} ).encode(''utf-8''), server_address_udp)');
                 Memo1.Lines.Add('                 sock_udp_arch.sendto(  json.dumps( {''name'':varname[i],''data'':''Error''} ).encode(''utf-8''), server_address_udp_arch)');
                 Memo1.Lines.Add('                 sock_udp_vk.sendto(  json.dumps( {''name'':varname[i],''data'':''Error''} ).encode(''utf-8''), server_address_udp_vk)');
 ...
 Memo1.Lines.Add('             sock_udp_vk = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)');
 Memo1.Lines.Add('             server_address_udp_vk = (''localhost'', 64002)');
...
begin
       ...
       begin
          if Length(StringGrid2.Cells[2, i]) > 0 then
            begin

            VkTree.items.AddChild(Node,StringGrid2.Cells[8, i]);
...
/////////////////
Memo1.Lines.Add(' <VkVariables>');
rowCountVk:= StringGrid3.RowCount-1;
       for i:=1 to RowCountVk do
       begin
            if   (Length(StringGrid3.Cells[1, i]) > 0) and
                 (Length(StringGrid3.Cells[4, i]) > 0) and
                 (Length(StringGrid3.Cells[3, i]) > 0)  then
                begin
                    Memo1.Lines.Add('    <VkPoint Id="'+i.ToString+'" VkVar="'+StringGrid3.Cells[1, i]+'" '+
                                        'VkMessage="'+StringGrid3.Cells[2, i]+'" VkId="'+StringGrid3.Cells[3, i]+
                    '" VkTime="'+StringGrid3.Cells[4, i]+'" >'+StringGrid2.Cells[3, i]+'</VkPoint>');
                end;
       end;
Memo1.Lines.Add(' </VkVariables>');

Memo1.Lines.Add(' <VkConfig>');
Memo1.Lines.Add('     <Cnf VkLogin="'+Edit15.Text+'" VkPassword="'+Edit16.Text+'"></Cnf>');
Memo1.Lines.Add(' </VkConfig>');
    ...
    begin
       with Child.ChildNodes do
           try
              for j := 0 to (Count - 1) do
                 begin
                     StringGrid3.RowCount:=Count+2;
                        for i := 0 to 4 do
                        begin
                           StringGrid3.Cells[0, j+1]:= (j+1).ToString;
                           if (Item[j].Attributes.Item[i].NodeName = 'VkVar') then       StringGrid3.Cells[1, j+1]:= String(Item[j].Attributes.Item[i].NodeValue);
                           if (Item[j].Attributes.Item[i].NodeName = 'VkMessage') then   StringGrid3.Cells[2, j+1]:= String(Item[j].Attributes.Item[i].NodeValue);
                           if (Item[j].Attributes.Item[i].NodeName = 'VkId') then        StringGrid3.Cells[3, j+1]:= String(Item[j].Attributes.Item[i].NodeValue);
                           if (Item[j].Attributes.Item[i].NodeName = 'VkTime') then      StringGrid3.Cells[4, j+1]:= String(Item[j].Attributes.Item[i].NodeValue);
...
begin
      ...
      //  Memo1.Lines.Add('from dateutil import parser');
        ...
        Memo1.Lines.Add('     if(json.loads( getData.decode("utf-8") )[''name''] == source):');
        Memo1.Lines.Add('         temp=json.loads( getData.decode("utf-8") )[''data'']');
...
begin
         ...
         Memo1.Lines.Add('echo ''Start python scripts''');
         Memo1.Lines.Add('nohup python3 '+ProjectPath+'modbus.py 1>/dev/null 2>/dev/null &');
         Memo1.Lines.Add('echo $! > '+ProjectPath+'modbus.pid');
         Memo1.Lines.Add('nohup python3 '+ProjectPath+'mclient.py 1>/dev/null 2>/dev/null &');
         Memo1.Lines.Add('echo $! > '+ProjectPath+'mclient.pid');
         Memo1.Lines.Add('nohup python3 '+ProjectPath+'jserver.py 1>/dev/null 2>/dev/null &');
         Memo1.Lines.Add('echo $! > '+ProjectPath+'jserver.pid');
         Memo1.Lines.Add('nohup python3 '+ProjectPath+'dbserver.py 1>/dev/null 2>/dev/null &');
         Memo1.Lines.Add('echo $! > '+ProjectPath+'dbserver.pid');
         Memo1.Lines.SaveToFile(ProjectPath+'start.sh' );
         Memo1.Clear;
         Memo1.Lines.Add('#!/bin/sh');
         Memo1.Lines.Add('echo ''Stop python scripts''');
         Memo1.Lines.Add('kill -9 `cat '+ProjectPath+'modbus.pid`');
         Memo1.Lines.Add('kill -9 `cat '+ProjectPath+'mclient.pid`');
         Memo1.Lines.Add('kill -9 `cat '+ProjectPath+'jserver.pid`');
         Memo1.Lines.Add('kill -9 `cat '+ProjectPath+'dbserver.pid`');
         Memo1.Lines.SaveToFile(ProjectPath+'stop.sh' );
...
begin
        ...
        Memo1.Lines.Add('###########################################################################');
        Memo1.Lines.Add('## Python code generated with '+Version+' '+ DateToStr(Date())+' ');
        Memo1.Lines.Add('## vk client by Jack Maclov');
        Memo1.Lines.Add('## http://scadapy.ln-group.ru/');
        Memo1.Lines.Add('##');
        Memo1.Lines.Add('## PLEASE DO ALL YOU NEED THIS FILE!');
        Memo1.Lines.Add('########################################################################### ');
       ...
       Memo1.Lines.Add('     return(mVar)');
        ...
        rowCount3:=StringGrid3.RowCount;
        for j3:=1 to rowCount3-1 do
           begin
           VarValue:= StringGrid3.Cells[1, j3];
           rowCount:=StringGrid2.RowCount;
              for j:=1 to rowCount-1 do
                begin
                   if(Length(StringGrid2.Cells[2, j])>0) and (Length(StringGrid2.Cells[9, j])>0) and (StringGrid2.Cells[8, j]=VarValue) then
                   Memo1.Lines.Add('     global '+StringGrid2.Cells[2, j]);
                end;
        ...
        begin
               ...
               for j:=1 to rowCount-1 do
               begin
                    if(Length(StringGrid2.Cells[2, j])>0) and (Length(StringGrid2.Cells[1, j])>0)  and (Length(StringGrid2.Cells[9, j])>0)  and (StringGrid2.Cells[8, j]=VarValue) then
                     begin
                        Memo1.Lines.Add('     '+StringGrid2.Cells[2, j]+'=setVariables('+StringGrid2.Cells[2, j]+',getData,'''+StringGrid2.Cells[1, j]+''','''+StringGrid2.Cells[2, j]+''','''+StringGrid2.Cells[3, j]+''','''+StringGrid2.Cells[4, j]+''','''+StringGrid2.Cells[5, j]+''','''+StringGrid2.Cells[6, j]+''','''+StringGrid2.Cells[7,j]+''')');
                        AllData:=AllData+StringGrid2.Cells[2, j]+','
                     end;
        ...
        Memo1.Lines.Add('                     if(int(temp[int(address)]) == 0):');
        Memo1.Lines.Add('                         temp2=False');
        Memo1.Lines.Add('                     varData=str(temp2)');
        Memo1.Lines.Add('             if(dataType==''string''):');
        Memo1.Lines.Add('                     temp2=temp[ int(address)]');
        Memo1.Lines.Add('                     varData=temp2');
        Memo1.Lines.Add('             try:');
        ...
        Memo1.Lines.Add('                 mult=float(multiply)');
        Memo1.Lines.Add('             if(dataType==''integer'' or dataType==''float''):');
        Memo1.Lines.Add('                 varData=str(varData*mult)');
        Memo1.Lines.Add('             else:');
        Memo1.Lines.Add('                 varData=str(varData)');
        ...
        Memo1.Lines.Add('     try:');
        Memo1.Lines.Add('         sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)');
        Memo1.Lines.Add('         server_address = (''localhost'', 64002)');
        Memo1.Lines.Add('         print(''starting up on {} port {}''.format(*server_address))');
        Memo1.Lines.Add('         sock.bind(server_address)');
        Memo1.Lines.Add('         while True:');
        Memo1.Lines.Add('             data, address = sock.recvfrom(4096)');
        ...
        Memo1.Lines.Add('     except Exception as e:');
        Memo1.Lines.Add('         print(e)');
        Memo1.Lines.Add('def sendMessage(user_id,putData,vklogin,vkpassword,message):');
        Memo1.Lines.Add('     vk = vk_api.VkApi(login = vklogin, password = vkpassword) ');
        Memo1.Lines.Add('     vk.auth() ');
        Memo1.Lines.Add('     nowTime=datetime.strftime(datetime.now(), "%d.%m.%Y %H:%M:%S")');
        Memo1.Lines.Add('     try:');
        Memo1.Lines.Add('         if(putData!=None and len(putData)>0):');
        Memo1.Lines.Add('             varName,varValue,varDataType=putData');
        Memo1.Lines.Add('             vk.method(''messages.send'', {''user_id'':user_id,''message'':nowTime+''\n''+message+'':''+varValue }) ');
        Memo1.Lines.Add('     except Exception as e: ');
        Memo1.Lines.Add('         print(e)');
        Memo1.Lines.Add('         pass ');
        Memo1.Lines.Add('def job1min():');
        ...
        begin
            ...
            //   try
               begin
                   if(Length(StringGrid2.Cells[2, j])>0) and (Length(StringGrid2.Cells[1, j])>0) and (StringGrid2.Cells[8, j]=VarValue) then
                   begin
                       if( StringGrid3.Cells[4, j3] = '1')  then
                         Memo1.Lines.Add('     sendMessage('''+StringGrid3.Cells[3, j3]+''','+StringGrid2.Cells[2, j]+','''+VkLogin+''','''
                         +VkPassword+''','''+StringGrid3.Cells[2, j3]+StringGrid3.Cells[1, j3]+''')');
                   end;
              end;
        //     except
        //   on E: Exception do
        ...
        begin
               ...
               begin
                   if(Length(StringGrid2.Cells[2, j])>0) and (Length(StringGrid2.Cells[1, j])>0) and (StringGrid2.Cells[8, j]=VarValue) then
                   begin
                       if( StringGrid3.Cells[4, j3] = '5')  then
                         Memo1.Lines.Add('     sendMessage('''+StringGrid3.Cells[3, j3]+''','+StringGrid2.Cells[2, j]+','''+VkLogin+''','''
                         +VkPassword+''','''+StringGrid3.Cells[2, j3]+StringGrid3.Cells[1, j3]+''')');
                   end;
                end;
        ...
        begin
               ...
               begin
                   if(Length(StringGrid2.Cells[2, j])>0) and (Length(StringGrid2.Cells[1, j])>0) and (StringGrid2.Cells[8, j]=VarValue) then
                   begin
                       if( StringGrid3.Cells[4, j3] = '15')  then
                         Memo1.Lines.Add('     sendMessage('''+StringGrid3.Cells[3, j3]+''','+StringGrid2.Cells[2, j]+','''+VkLogin+''','''
                         +VkPassword+''','''+StringGrid3.Cells[2, j3]+StringGrid3.Cells[1, j3]+''')');
                   end;
                end;
        ...
        begin
               ...
               begin
                   if(Length(StringGrid2.Cells[2, j])>0) and (Length(StringGrid2.Cells[1, j])>0) and (StringGrid2.Cells[8, j]=VarValue) then
                   begin
                       if( StringGrid3.Cells[4, j3] = '30')  then
                         Memo1.Lines.Add('     sendMessage('''+StringGrid3.Cells[3, j3]+''','+StringGrid2.Cells[2, j]+','''+VkLogin+''','''
                         +VkPassword+''','''+StringGrid3.Cells[2, j3]+StringGrid3.Cells[1, j3]+''')');
                   end;
                end;
        ...
        begin
               ...
               begin
                   if(Length(StringGrid2.Cells[2, j])>0) and (Length(StringGrid2.Cells[1, j])>0) and (StringGrid2.Cells[8, j]=VarValue) then
                   begin
                       if( StringGrid3.Cells[4, j3] = '60')  then
                         Memo1.Lines.Add('     sendMessage('''+StringGrid3.Cells[3, j3]+''','+StringGrid2.Cells[2, j]+','''+VkLogin+''','''
                         +VkPassword+''','''+StringGrid3.Cells[2, j3]+StringGrid3.Cells[1, j3]+''')');
                   end;
                end;
        ...
        Memo1.Lines.Add('     AllData=''''');
        Memo1.Lines.Add('     try: ');
        Memo1.Lines.Add('         print( ''Starting VK Client...'')');
        Memo1.Lines.Add('         modb = threading.Thread(target=udpserv,args=(1,))');
        Memo1.Lines.Add('         modb.daemon = True');
        Memo1.Lines.Add('         modb.start()');
        Memo1.Lines.Add('         arch = threading.Thread(target=startJob,args=(1,))');
        ...
        Memo1.Lines.Add('         print(e)');
        try
            Memo1.Lines.SaveToFile(ProjectPath+'vkclient.py');
          except
             ShowMessage('Error to save');
          end;
