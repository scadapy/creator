...
def Proc_4(f=1):
     ...
     for i in range(0,unitCount+1):
         ...
         timeOut.append(i)
     try:
         com=serial.Serial('com37',9600, bytesize=8, parity='N', stopbits=1, xonxoff=0)
     ...
     while True:
         for i in range(0,unitCount):
             try:
                 ...
                 sock_udp_arch.sendto((varname[i]+":"+str(varNameData)).encode('utf-8'), server_address_udp_arch)
             except Exception as e:
                 varNameData=None
                 sock_udp.sendto((varname[i]+":Error").encode('utf-8'), server_address_udp)
                 sock_udp_arch.sendto((varname[i]+":Error").encode('utf-8'), server_address_udp_arch)
...
def Proc_5(f=1):
     ...
     try:
         ...
         master.set_timeout(2)
          ...
          pass
     x=0; varname[x],rtuAddress[x],reg[x],startAdr[x],rangeAdr[x],timeOut[x]=('Generator_TC','1','READ_DISCRETE_INPUTS','0','11','1')
     while True:
         for i in range(0,unitCount):
             try:
                 if(reg[i]=='READ_INPUT_REGISTERS'):   c=cst.READ_INPUT_REGISTERS
